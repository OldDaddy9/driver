...
void adlak_pm_enable(void *padlak, uint32_t en) {
    ...
    if (d_rev.bitc.major_rev >= 3) {
        ...
        */
        adlak_write32(region, REG_ADLAK_PM_UNIT, 0x05);  // 32bit
...
static void adlak_hal_get_revision(void *data) {
    ...
    AML_LOG_INFO("ADLAK HW Revision:%d.%d,REG_ADLAK_CLK_IDLE_CNT=0x%08X", phw_info->rev.bitc.major,
                 phw_info->rev.bitc.minor, adlak_read32(region, REG_ADLAK_CLK_IDLE_CNT));
}
...
static int adlak_hal_set_autoclock(void *data, uint32_t en) {
    ...
    if (0 == en) {
        ...
        adlak_write32(region, REG_ADLAK_CLK_AUTOCLK, 0x3FFF);
        adlak_write32(region, REG_ADLAK_MC_CTL, 0xDF);
        adlak_write32(region, REG_ADLAK_CLK_IDLE_CNT, 0x0608);
    }
    return 0;
}
