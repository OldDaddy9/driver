...
#define LIST_POISON2 (0x122 + POISON_POINTER_DELTA)
...
#else
static inline bool __list_add_valid(struct list_head *new, struct list_head *prev,
                                    struct list_head *next) {
    return true;
}
static inline bool __list_del_entry_valid(struct list_head *entry) { return true; }
#endif
 ...
 */
static inline void list_bulk_move_tail(struct list_head *head, struct list_head *first,
 ...
 * @list.
 */
static inline void list_cut_before(struct list_head *list, struct list_head *head,
                                   struct list_head *entry) {
    if (head->next == entry) {
        INIT_LIST_HEAD(list);
        return;
